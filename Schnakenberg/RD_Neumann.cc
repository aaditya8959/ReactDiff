/* SCHNAKENBERG REACTION_DIFFUSION SYSTEM
 *
...
namespace RD_Schnakenberg
{
  ...
  // Read from parameters file
	void parse_command_line (const int     argc,
         char *const *argv)
		 {
			...
			apar = prm.get_double("apar") ;
			bpar = prm.get_double("bpar") ;
			dpar = prm.get_double("diffusivity") ;
			...
			x_subd = prm.get_integer("x subdivisions") ;
			y_subd = prm.get_integer("y subdivisions") ;
			time_step = prm.get_double("time step") ;
			total_time = prm.get_double("total time") ;
			output_timestep_skip = prm.get_integer("skip frequency") ;
  ...
  // Setting up the problem - mesh, constraints, initializing solution variables
  template <int dim>
  ...
  { 
	...
	std::vector<unsigned int> subdiv_list ;
	  subdiv_list.push_back(x_subd) ;
	  subdiv_list.push_back(y_subd) ;
	  
	  GridGenerator::subdivided_hyper_rectangle(triangulation, subdiv_list,
